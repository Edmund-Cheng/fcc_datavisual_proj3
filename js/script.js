...
function callback(data) {
  ...
  var fontSize = 16;
  // each column width = 5. Each years has 12 month. Total years = the data count / 12. So width = 5 * ceiling (all data / 12)
  ...
  svg
    ...
    .attr(
      'transform',
      'translate(' + -5 * fontSize + ',' + height / 2 + ')' + 'rotate(-90)'
    )
    .attr('fill', 'black');
  ...
  var xAxis = d3
    ...
    .tickValues(
      xScale.domain().filter(function (year) {
        // set ticks to years which is divisible by 10
        return year % 10 === 0;
      })
    )
